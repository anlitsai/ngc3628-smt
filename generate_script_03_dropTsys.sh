...
file_script='dropTsys.class'
file_drop0='drop.list0'

file_class_in[0]=`ls | grep n3628 | grep smt-f1m.smt` 
file_class_in[1]=`ls | grep n3628 | grep smt-fqm.smt` 

file_class_root_in[0]=`echo ${file_class_in[0]} | cut -d . -f1` 
file_class_root_in[1]=`echo ${file_class_in[1]} | cut -d . -f1` 

file_class_out[0]=`echo ${file_class_root_in[0]}'_dropTsys.smt'` 
file_class_out[1]=`echo ${file_class_root_in[1]}'_dropTsys.smt'` 
...
j=0
  ...
  scan_drop=`cat $file_drop0 | egrep 'Tsys|wrong_position' | awk '{print $1}'`
